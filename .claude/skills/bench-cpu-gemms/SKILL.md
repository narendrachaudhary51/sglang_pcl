...
- Write these shapes into a references/${model_name}/gemm_shapes.txt file. This file will be used as input for the benchmark script in the next step.
...
- Read the shapes from the N and K parameters from references/${model_name}/gemm_shapes.txt file to get the data for each shape.
- Run the scripts/run_gemm_sweep.py with M parameter list as input. This script will run the benchmark for each shape and precision and write the results into a references/${model_name}/gemm_benchmark_results.csv file. 
...
- Write the shape, precision, TFLOPs and bandwidth into a references/${model_name}/gemm_benchmark_results.csv file.

# Step 5: Plot the results

- Plot the results from the references/${model_name}/gemm_benchmark_results.csv file using the scripts/${model_name}/plot_gemm_results.py script. This script takes model name as an input. This script will generate plots for each precision and save them in the references/plots directory. The plots will show the performance in TFLOPs and bandwidth in GB/s for each shape and precision.
