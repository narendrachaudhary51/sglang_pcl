...
# CPUQuantMethod.INT8_W8A8 (see sgl-kernel/csrc/cpu/gemm.h)
INT8_W8A8 = 1
# Per-channel int8 quantization (matches real model configs, e.g. Qwen3 W8A8)
...
def fused_moe(a, packed_w1, packed_w2, w1s, w2s, score, topk, renormalize, prepack):
    ...
    fused_output = fused_experts_cpu(
        ...
        None,  # w1_zp
        None,  # w2_zp
        None,  # block_size (per-channel, not block-wise)
        None,  # w1 bias
        None,  # w2 bias
        None,  # alpha
        None,  # limit
...
def bench_int8_moe(M, N, K, num_experts, topk=8):
    ...
    bytes_per_layer = (
        M * K * 2      # a: bf16
        + (M*topk if M*topk < num_experts else num_experts) * (2*N) * K * 1  # up_packed: int8
        + (M*topk if M*topk < num_experts else num_experts) * K * N * 1    # down_packed: int8
        + (M*topk if M*topk < num_experts else num_experts) * (2*N) * 4  # w1s: fp32 per-channel
        + (M*topk if M*topk < num_experts else num_experts) * K * 4  # w2s: fp32 per-channel
    )
    ...
    for _ in range(num_layers):
        ...
        w1_fp32 = (torch.rand(num_experts, 2 * N, K) - 0.5) * 2
        w1 = (w1_fp32 * int8_max).clamp(min=int8_min, max=int8_max).to(torch.int8)

        w2_fp32 = (torch.rand(num_experts, K, N) - 0.5) * 2
        w2 = (w2_fp32 * int8_max).clamp(min=int8_min, max=int8_max).to(torch.int8)

        # Per-channel (per-output-column) scales
        w1s = torch.rand(num_experts, 2 * N) * int8_factor_for_scale
        w2s = torch.rand(num_experts, K) * int8_factor_for_scale
...
if __name__ == "__main__":
    ...
    bench_int8_moe(M, N, K, num_experts=E, topk=topk)
