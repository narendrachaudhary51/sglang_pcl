...
def get_dataset(args, tokenizer, model_id=None):
    ...
    elif args.dataset_name == "custom_hf":
        assert not tokenize_prompt
        input_requests = sample_custom_hf_requests(
            dataset_path=args.dataset_path,
            num_requests=args.num_prompts,
            tokenizer=tokenizer,
            fixed_output_len=args.sharegpt_output_len,
...
def sample_custom_hf_requests(
    ...
    from pathlib import Path
    assert Path(dataset_path).is_dir() and \
        (Path(dataset_path)/"dataset_info.json").exists()
    from datasets import load_from_disk
    data = load_from_disk(dataset_path)
    data = data.shuffle(seed=seed)  # Shuffle the dataset with a fixed seed for reproducibility
    # Filter out sequences that are too long or too short
    filtered_dataset: List[DatasetRow] = []
    ...
    for item in data:
        ...
        reference_answer = item["ref_output"]
        messages = [
